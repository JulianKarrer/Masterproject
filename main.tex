...
\chapter{Smoothed Particle Hydrodynamics}\label{chp:sph}
    ...
    \subsubsection*{Particle Deficiency and Free Surfaces}
...
\chapter{Incompressible Fluid Solver}\label{chp:fluid}
  With the SPH method from \autoref{chp:sph}, the tools to discretize and numerically solve the Navier-Stokes equations outlined in \autoref{sec:navier-stokes} are available. This chapter focuses more concretely on implementing such a fluid solver and facing the challenge of ensuring the continuity equation is upheld by an incompressible fluid. This leads to the pressure Poisson equation or PPE for short, which is iteratively solved by the Implicit Incompressible SPH solver of \autoref{sec:iisph}. Multiple source terms and variations of the solver are available, which are discussed in \autoref{sec:alternative-source-terms}.
    ...
    \end{align}
    before implicitly integrating the pressure acceleration $\vek{a}_i^p$ as described in the following. This method is therefore named \emphasis{Implicit Incompressible SPH} or IISPH for short and was proposed by \autocite[Ihmsen et al.]{iisph}

    \subsubsection*{The Pressure Poisson Equation}

    Pressure computation is, in the case of inviscid flows, the stiff subproblem in the partial differential equations to solve \autocite{tutorial2019}. Pressure values that strongly enforce incompressibility, or in other words act as Lagrange multipliers to the continuity equation \autoref{eq:continuity} \autocite{tutorial2019}, should be computed efficiently. This can be done using iterative, global solvers that solve the \emphasis{Pressure Poisson equation} or PPE for short, which asserts that the pressure forces do in fact enforce the continuity equation. 
    ...
    \subsubsection*{Coefficient Extraction and Diagonal Element}
    With the discretized pressure Poisson equation obtained, one can go about solving the resulting system of $N$ equations at $N$ particles for the unknown pressure values $p_i$. While many possible solvers for this problem exist, a relatively straightforward implementation that is robust, matrix-free, i.e. does not rely on explicitly constructing system matrices of prohibitive $\mathcal{O}\br{N^2}$ proportions, and which can be fully parallelized is achieved by a \emphasis{Relaxed Jacobi Solver} as suggested by the \autocite[original authors of the method]{iisph}. 
    ...
    \begin{align}
      ...
      &\textit{factor out $p_i$}
      % &= p_i \vek{c}_i - \sum_{j\in\mathcal{N}_i\setminus\{i\}} 
      %   m_j\frac{p_j}{\rho_i^2} \nabla W_{ij} 
      % &\textit{$\nabla W_{ii}=0$ due to \autoref{eq:sph-property-symmetry}}
    \end{align}
    where since the kernel is spherically symmetric (\autoref{eq:sph-property-symmetry}) and therefore $\nabla W_{ii}=0$ one can exclude the index $i$ from the set of neighbours and write:
    \begin{align}
      \vek{a}_i^p &= p_i \vek{c}_i - 
      \sum_{j\in\mathcal{N}_i\setminus\{i\}} 
        m_j\frac{p_j}{\rho_i^2} \nabla W_{ij} \label{eq:iisph-a-p-c_i}
    \end{align}
    which makes it easier to factor out the $p_i$ in the larger expression $\br{\mathds{A}\vek{p}}_{i}$ as follows:
    \begin{align}
    ...
    \begin{align*}
      \br{\mathds{A}\vek{p}}_{i}={}
        &\Delta t^2 \sum_{j\in\mathcal{N}_i} m_j 
         \br{p_i \vek{c}_i 
         \underbrace{- \sum_{j\in\mathcal{N}_i\setminus\{i\}} m_j\frac{p_j}{\rho_i^2} \nabla W_{ij}}_{\textit{does not contribute, $j\neq i$}}
         } 
         \cdot \nabla W_{ij}
       ...
       &-\Delta t^2 \sum_{j\in\mathcal{N}_i} m_j 
         \br{
           ...
           \underbrace{- \sum_{l\in\mathcal{N}_j\setminus\{j\}} m_l\frac{p_l}{\rho_j^2} \nabla W_{jl}}_{\textit{only contributes for $l=i$}}
         }
         \cdot \nabla W_{ij}
         &{}\\
    ...
    \equationnamed{Diagonal Element $\mathds{A}_{ii}$}{
      \begin{align}
        ...
        \textit{where: \quad}\vek{c}_i &:= -\sum_{j\in\mathcal{N}_i} 
        \frac{m_j}{\rho_i^2} \nabla W_{ij} 
       -\sum_{k\in\mathcal{B}_i} 
        \frac{m_k}{\rho_i^2} \nabla W_{ik} 
      \end{align}
    ...
    It is also worth noting that both the specific boundary handling method described in \autoref{chp:rigid}, namely \autocite[consistent boundaries]{consistent-boundaries} with density mirroring, as well as the semi-implicit time integration scheme used in \autoref{eq:numerical-time-integration} and the operator splitting in \autoref{eq:operator-splitting} are baked into the expression for $\mathds{A}_{ii}$ and the diagonal element needs to be adjusted if any of these factors differ.
  ...
  % The two constraints (\autoref{enum:continuity-equation-constraint-den} and \autoref{enum:continuity-equation-constraint-vel}) derived from the continuity equation result in two common source terms for the PPE \autocite{tutorial2019}:
  % \begin{align}
  %   \Delta t \Laplace \vek{p}_i &= \frac{1}{\Delta t}\br{\rho_o - \rho_i^*} 
  %   &\textit{density invariance}\\
  %   \Delta t \Laplace \vek{p}_i &= \rho_i \divergence \vek{v}_i^*
  %   &\textit{divergence free velocities}
  % \end{align}
  % where $\rho_i^*$ is the predicted density due to non-pressure accelerations. We focus for now on the former source term,
  
  
  % This means that the predicted velocity $\vek{v}_i^*$ causes some measured density $\rho_i^*$ that might exceed the reference rest density $\rho_0$, in which case the pressure forces solving the above equation project the predicted state onto an uncompressed state in which $\rho_i\br{t+\Delta t}\approx\rho_0$. 
...
for (int i = 0; i < P.length; ++i) {
  ...
  if (isNormalTowardsO(A, B, O)) {
    ...
    \end{align}
    in terms of the initial inertia tensor $\mathds{I}\br{t=0}$ that was computed in \autoref{subsubsec:inertia-tensor-computation}. Note that since $\mathds{O}$ is a rotation matrix, the transpose of the matrix is its inverse. 
