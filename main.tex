...
\chapter{Weakly Coupled Rigid Bodies}\label{chp:rigid}
    ...
    The method makes use of the notion of signed volumes $V'$ of tetrahedra, where given some vertex $O$ of a tetrahedron and the triangle $\triangle_{ABC}$ formed by its remaining, ordered vertices, it is assigned either a positive or a negative volume depending on whether the normal of the triangle $\triangle_{ABC}$ faces towards or away from $O$. Note that since the input mesh is now a watertight manifold, each connected component of the mesh defines a bounded interior and unbounded exterior, consists of triangles and can therefore be stored in a consistent winding order such that the normals $\frac{\overrightarrow{AB} \times \overrightarrow{AC}}{\abs{\overrightarrow{AB} \times \overrightarrow{AC}}}$ all point towards the exterior or all point towards the interior. In this instance, counter-clockwise winding is assumed, but clockwise winding can be accounted for by flipping the sign of the signed volume.
...
for (int i = 0; i < P.length; ++i) {
  ...
  if (isNormalTowardsO(A, B, O)) {
    ...
    \end{equation}
    From now on, it will be assumed that the winding order and the order of the vectors in \autoref{eq:tetrahedron-signed-volume-determinant} were chosen such that the sum of signed volumes are always positive and the absolute value $\abs{\cdot}$ in \autoref{eq:tetrahedron-signed-volume-sum} could be dropped.

    Better yet, this method generalizes to any mass moments as defined in \autoref{eq:mass-density-moments} through the relation \autocite{efficient-feature-extraction}:
    \begin{equation}\label{eq:mass-density-moments-using-signed-volume}
      \mathcal{M}_{pqr} = \sum_{\triangle_{ABC}} \br{V'\br{\tetrahedron_{ABCO}} \cdot \iiint_{\Omega\br{ABCO}} \rho x^p y^q z^r  \,dV}
    \end{equation}
    where $\Omega\br{ABCO}$ is the domain of the tetrahedron $\tetrahedron_{ABCO}$, for which exact analytic solutions to the triple integral exist when homogeneous density is assumed.
    ...
    \subsubsection{First mass density moments: centre of mass}
    ...
    \end{align}
    and analogously for the other two axes. These exact analytic expressions are cited from \autocite[Tonon]{explicit-exact-tetrahedron-formulas} and moved to \autoref{chp:analytic-tetrahedron-inertia} since they are rather verbose.
...
\begin{appendices}
  ...
  \begin{align}
    ...
    I_{xz} &= \mathcal{M}_{100} + \mathcal{M}_{001}\\
    I_{yz} &= \mathcal{M}_{010} + \mathcal{M}_{001}\\
    \mathcal{M}_{pqr} &= \sum_{\triangle_{ABC}} \br{V'\br{\tetrahedron_{ABCO}} \cdot \underbrace{\iiint_{\Omega\br{ABCO}} \rho x^p y^q z^r  \,dV}_{\text{unknown so far}}}\label{eq:appendix-tetra-mass-moment}
  \end{align}
  As can be seen, since the signed volume $V'$ of the tetrahedron $\tetrahedron_{ABCO}$ can be computed as outlined in \autoref{sec:mass-moments}, only the volume integral in \autoref{eq:appendix-tetra-mass-moment} is yet to be explained. Exact analytic formulas for this integral in terms of the vertex coordinates $\vek{A},\vek{B},\vek{C}$ were taken from \autocite[Tonon]{explicit-exact-tetrahedron-formulas}, with the only difference being that a minor error in the paper ($b'$ and $c'$ are swapped) was fixed and that by using the origin as one of the four vertices, some terms drop out:
  ...
  \end{align}
  and so on for the other values, where $\rho \abs{\det(J\br{\tetrahedron_{ABCO}})} $ was moved to the right-hand side to save space. 
